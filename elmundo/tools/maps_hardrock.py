...
shapefile_path2 = os.path.join(INPUT_DIR2, "Hardrock_Shapefile_4326.shp")
...
for _, site_point in excluded_sites_gdf.iterrows():   # You can select how many sites to process here
...
def plot_closest_points(df, state_data, geologic_storage_data, site_gdf, cmap, norm):
    ...
    for _, row in df.iterrows():
        ...
        # Plot the site point
        ax.plot(site_point.x, site_point.y, marker='o', color=color, markersize=5)

        # Plot the closest point
        ax.plot(closest_point.x, closest_point.y, marker='x', color=color, markersize=2)
 ...
 #   color = cmap(norm(row['polygon_distance']))
  #  site_point = site_gdf.loc[row['site_name']].geometry
   # closest_point = row['closest_point']
    ...
    # Plot the closest point
    #ax1.plot(closest_point.x, closest_point.y, marker='x', color=color, markersize=2)
...
from breakpoint import combined_cost_salt_cavern_compressor_pipeline, combined_cost_underground_pipe_compressor, find_equilibrium_pipeline_length, combined_cost_hardrock_cavern_compressor_pipeline

# Apply the function directly using a lambda function
final_closest_points_df['hardrock_cavern_cost'] = final_closest_points_df.apply(
    lambda row: combined_cost_hardrock_cavern_compressor_pipeline(
...
print(final_closest_points_df.head())
final_closest_points_df.to_csv("/Users/mmoulton/Documents/Projects/Elmundo/inputs/{}hardrock_sites_final_sitelist.csv".format(len(final_closest_points_df)))
...
within_storage_sites['hardrock_cavern_cost'] = within_storage_sites.apply(
    lambda row: combined_cost_hardrock_cavern_compressor_pipeline(
        row['hydrogen_storage_size_kg'],
        100,  # p_outlet fixed at 100
        row['max_h2_kg_pr_hr'] * 24,  # Convert flow rate to kg/day
...
within_storage_sites.to_csv("/Users/mmoulton/Documents/Projects/Elmundo/inputs/{}hardrock_sites_within_storage_sites.csv".format(len(within_storage_sites)))
